...
export function ProjectCard({ index, project }: Props) {
  return (
    <article className="project-card group flex min-w-0 h-full flex-col overflow-hidden rounded-2xl border border-border bg-card p-5 transition hover:border-accent sm:p-6">
      <div className="project-card-media"><ProjectVisual slug={project.slug} priority={index === 1} /></div>
      ...
      <h3 className="mt-5 break-words font-display text-2xl leading-tight tracking-[-0.025em] text-foreground">
        {project.title}
      </h3>
      <p className="mt-3 line-clamp-3 text-sm leading-relaxed text-muted">{project.description}</p>

      <div className="mt-5 mb-6 flex flex-wrap gap-x-4 gap-y-2 font-mono text-[0.64rem] uppercase tracking-[0.1em] text-muted">
